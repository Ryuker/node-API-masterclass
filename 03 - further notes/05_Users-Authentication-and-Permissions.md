...
## UpdateDetails handler and route
- Added `updateDetails` to `controllers/auth.js`
``` JS controllers/auth.js
...
## getUser handler (for single user)
...
exports.getUser = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return new ErrorResponse(`User ${req.params.id} not found`, 400);
  }
...
## Users routes
- Added users routes to `routes/users.js`
``` JS routes/users.js
const express = require('express');
...
- Then brought the `users` routes middleware into `server.js`
